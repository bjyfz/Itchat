...
from model.transformers import (WEIGHTS_NAME, BertConfig,
                                    BertForSequenceClassification, BertTokenizer, BertModel,
                                    RobertaConfig, XLNetConfig,
                                    XLNetForSequenceClassification,
                                    XLNetTokenizer,
                                    AlbertForSequenceClassification)
from model.transformers import AdamW, WarmupLinearSchedule
    ...
    for _ in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.model_type != 'distilbert':
                inputs['token_type_ids'] = batch[2].to(args.device) if args.model_type in ['bert', 'xlnet', 'albert',
...
def repre(args, model, tokenizer, label_list, prefix=""):
    sent_repres = []
    pred_task_names = (args.task_name,)
    pred_outputs_dirs = (args.output_dir,)
    label_map = {i: label for i, label in enumerate(label_list)}
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        pred_dataset = load_and_cache_examples(args, pred_task, tokenizer, data_type='run_case')
        if not os.path.exists(pred_output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(pred_output_dir)
        ...
        logger.info("******** Running representation {} ********".format(prefix))
        logger.info("  Num examples = %d", len(pred_dataset))
        logger.info("  Batch size = %d", args.pred_batch_size)
        nb_pred_steps = 0
        preds = None
        ...
        for step, batch in enumerate(pred_dataloader):
            ...
            with torch.no_grad():
                ...
                if args.model_type != 'distilbert':
                    inputs['token_type_ids'] = batch[2] if (
                            'bert' in args.model_type or 'xlnet' in args.model_type) else None  # XLM, DistilBERT and RoBERTa don't use segment_ids
                hidden_reps, cls_reps = model(**inputs)
                sent_repres.extend(cls_reps.tolist())
                print("hidden_reps: ", len(hidden_reps[0]))
                print("cls_reps: ", cls_reps.tolist())
        return sent_repres
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    if os.path.exists(cached_features_file):
        ...
        if data_type == 'train':
            ...
            examples = processor.get_run_case_examples(args.data_dir)
...
def main():
    ...
    parser.add_argument("--do_repre", action='store_true',
                        help="Whether to run the model in inference mode on the test set.")
    ...
    config_class, model_class, tokenizer_class, bert_class = MODEL_CLASSES[args.model_type]
    ...
    if args.do_repre and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        checkpoints = [args.output_dir]
        if args.predict_checkpoints > 0:
            checkpoints = list(
        ...
        for checkpoint in checkpoints:
            prefix = checkpoint.split('/')[-1] if checkpoint.find('checkpoint') != -1 else ""
            model = bert_class.from_pretrained(checkpoint)
            model.to(args.device)
            sent_repre = repre(args, model, tokenizer, label_list, prefix=prefix)
