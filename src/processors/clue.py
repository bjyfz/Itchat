...
def clue_convert_examples_to_features(examples,
                                      tokenizer,
                                      max_length=512,
                                      task=None,
                                      label_list=None,
...
class AfqmcProcessor(DataProcessor):
    ...
    def get_run_case_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_json(os.path.join(data_dir, "run_case.json")), "run_case")

    def get_input_data_examples(self, data):
        """See base class."""
        return self._create_examples(data, "input_data")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            if "sentence2" in line:
                text_b = line['sentence2']
            else:
                text_b = None
            label = str(line['label']) if (set_type != 'test' and set_type != 'run_case' and set_type != 'input_data') else "0"
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
